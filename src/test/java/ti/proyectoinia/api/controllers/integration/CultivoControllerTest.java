...
class CultivoControllerTest {
    ...
    private final String baseUrl = "/api/v1/cultivo";
    ...
    void getById_ReturnsOk() throws Exception {
        ...
        Mockito.when(service.obtenerCultivoPorId(1L)).thenReturn(dto);
    ...
    void crear_Correcto() throws Exception {
        ...
        CultivoDto input = new CultivoDto();
        input.setNombre("Felipe");


        Mockito.when(service.crearCultivo(any(CultivoDto.class)))
    ...
    void crear_YaExiste() throws Exception {
        ...
        Mockito.when(service.obtenerCultivoPorId(input.getId())).thenReturn(input);
    ...
    void update_ReturnsUpdated() throws Exception {
        ...
        Mockito.when(service.editarCultivo(any(CultivoDto.class))).thenReturn("Creado");
    ...
    void delete_ReturnsOk() throws Exception {
        ...
        Mockito.verify(service).eliminarCultivo(1L);
    }
    ...
    void delete_ReturnsNotFound() throws Exception {
        ...
        Mockito.doThrow(new EntityNotFoundException("No existe"))
                .when(service).eliminarCultivo(2L);

        mockMvc.perform(delete(baseUrl + "/eliminar/2")
                        .with(csrf()))
                .andExpect(status().isNotFound());

        Mockito.verify(service).eliminarCultivo(2L);
    }
    ...
    void get_ReturnsOk() throws Exception {
        ResponseListadoCultivos dto = new ResponseListadoCultivos();
        Mockito.when(service.listado()).thenReturn(ResponseEntity.ok(dto));
